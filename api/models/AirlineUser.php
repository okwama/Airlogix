...
class AirlineUser {
    ...
    public function getProfile($user_id) {
        $query = "SELECT id, phone_number, email, first_name, last_name, date_of_birth, nationality, passport_number, passport_expiry_date, frequent_flyer_number, profile_photo_url, created_at, member_club, loyalty_points 
    ...
    public function savePushSubscription($user_id, array $data) {
        $provider = strtolower(trim((string)($data['provider'] ?? 'onesignal')));
        ...
        if ($subscriptionId === '') {
            return ['status' => false, 'message' => 'Subscription ID is required'];
        }

        if (!in_array($platform, ['android', 'ios', 'web'], true)) {
            return ['status' => false, 'message' => 'Invalid platform'];
        }
        ...
        $query = "INSERT INTO push_subscriptions (user_id, provider, platform, subscription_id, external_user_id, status)
                  VALUES (:user_id, :provider, :platform, :subscription_id, :external_user_id, :status)
        ...
        try {
            if ($stmt->execute([
                ...
                return ['status' => true, 'message' => 'Push subscription saved successfully'];
            }
            return ['status' => false, 'message' => 'Failed to save push subscription'];
        } catch (PDOException $e) {
    ...
    public function deactivatePushSubscription($user_id, array $data) {
        $provider = strtolower(trim((string)($data['provider'] ?? 'onesignal')));
        $subscriptionId = trim((string)($data['subscription_id'] ?? ''));
        ...
        $query = "UPDATE push_subscriptions
                  ...
                  WHERE user_id = :user_id AND provider = :provider AND subscription_id = :subscription_id";
        $stmt = $this->conn->prepare($query);
        ...
        try {
            ...
            return ['status' => true, 'message' => 'Push subscription deactivated'];
        } catch (PDOException $e) {
            error_log("Failed to deactivate push subscription: " . $e->getMessage());
            return ['status' => false, 'message' => 'Database error'];
        }
    }
