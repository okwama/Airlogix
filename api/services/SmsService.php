...
class SmsService {
    ...
    public function send(string $toNumber, string $message): bool {
        ...
        // Normalize Kenyan local format 07xxxxxxxx / 01xxxxxxxx to +254xxxxxxxxx
        if (preg_match('/^0((?:7|1)\d{8})$/', $toNumber, $matches)) {
            $toNumber = '+254' . $matches[1];
        } elseif (!str_starts_with($toNumber, '+')) {
