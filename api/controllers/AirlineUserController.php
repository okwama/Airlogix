...
class AirlineUserController {
    ...
    public function registerPushSubscription() {
        ...
        if (empty($data['subscription_id'])) {
            Response::fail(400, 'Subscription ID is required', 'PUSH_SUBSCRIPTION_REQUIRED');
            return;
        }

        $response = $this->userModel->savePushSubscription($user_id, $data);
        if (!empty($response['status'])) {
            Response::json($response);
            return;
        }

        Response::fail(400, (string)($response['message'] ?? 'Failed to save push subscription'), 'PUSH_SUBSCRIPTION_SAVE_FAILED');
    }

    public function unregisterPushSubscription() {
        $headers = request_headers();
        $token = isset($headers['Authorization']) ? str_replace('Bearer ', '', $headers['Authorization']) : '';
        $user_id = $this->userModel->validateToken($token);
        ...
        $response = $this->userModel->deactivatePushSubscription($user_id, $data);
        if (!empty($response['status'])) {
            Response::json($response);
            return;
        }

        Response::fail(400, (string)($response['message'] ?? 'Failed to deactivate push subscription'), 'PUSH_SUBSCRIPTION_DEACTIVATE_FAILED');
    }
