...
class FlightController {
    ...
    public function search() {
        ...
        // Always search for related/upcoming dates or nearby alternative airports
        $allSuggestions = $this->flightModel->searchRelated($fromIds, $toIds, $date);
        
        // Filter out any suggestion that is already in the main search results
        $flightIds = array_column($flights, 'id');
        $suggestions = array_values(array_filter($allSuggestions, function($s) use ($flightIds) {
            return !in_array($s['id'], $flightIds);
        }));
